(() => {
        ...
        (room, index) => `
          <li class="room-card" data-room-card="${room.id}">
            <div class="video-shell">
              <video
                id="overview-video-${room.id}"
                playsinline
                autoplay
                muted
                preload="metadata"
                aria-label="${room.name}直播画面"
              ></video>
              ...
              <button
                class="card-open"
                type="button"
                data-open-room="${index}"
                aria-label="打开${room.name}"
              >
                <span>查看大画面</span>
              </button>
            </div>
      ...
      .map(
        (room, index) => `
          <button
            ...
            type="button"
            data-select-room="${index}"
            data-room-list="${room.id}"
  ...
  function setStatus(room, state, label) {
    ...
    if (currentView === "focus" && rooms[selectedIndex].id === room.id) {
      ...
      if (state !== "live") {
        const title = elements.focusPlaceholder.querySelector("strong");
        const detail = elements.focusPlaceholder.querySelector("small");
        if (title) {
          title.textContent =
            state === "connecting" ? "正在连接直播信号" : "该直播间暂未开播";
        }
        if (detail) detail.textContent = "页面会在后台自动重试";
      }
    }
  ...
  function updateLiveCount() {
    const count = [...roomStates.values()].filter((state) => state === "live").length;
  ...
  function toggleSound() {
    const video = elements.focusVideo;
    video.muted = !video.muted;
    elements.soundButton.textContent = video.muted ? "开启声音" : "关闭声音";
  ...
  function toggleFullscreen() {
    if (!document.fullscreenElement) {
      elements.focusStage.requestFullscreen?.().catch(() => {
